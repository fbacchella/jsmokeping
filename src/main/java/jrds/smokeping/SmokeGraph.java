...
public class SmokeGraph extends jrds.Graph {
    ...
    protected void setGraphDefData(RrdGraphDef graphDef, Probe<?, ?> defProbe, ExtractInfo ei,
        ...
        graphDef.setMaxValue(getMax(defProbe, ei) * 1.2);
        graphDef.setDownsampler(null);

        graphDef.datasource("stddev", "median", new Variable.STDDEV());

        for (int i = 1 ; i <= 10; i++){
            String rpn = String.format("ping%d,UN,UNKN,ping%d,ping%d,-,IF", i, 21 - i, i);
            graphDef.datasource("smoke" + i, rpn);
            graphDef.area("ping" + i, TRANSLUCENT);
            graphDef.stack("smoke" + i, GRAYS[i - 1]);
        }

        graphDef.comment("\\l");
        graphDef.gprint("stddev", "Median standard deviation ± %.1lf %ss\\l");
        ...
        graphDef.line("median", MEDIUM_GREEN);
    }
    ...
    private static final Color MEDIUM_GREEN = new Color(0x029602);
